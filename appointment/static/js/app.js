    ...
    return {
        ...
        updateOnlieUsers : function (users) {
            ...
            for (var i = 0; i < users.length; i++) {
                ...
                console.log(userlst);
                this.list = $('.onlinepeers'+users[i].replace(' ', ''));
                if (this.list){
                    this.list.empty();
                    var usr = '<button type="button" class="peeruser">'+ users[i] + '</button>'
                    this.list.append(usr);
                }
                // if(available_patients.pateint_name == userlst){
